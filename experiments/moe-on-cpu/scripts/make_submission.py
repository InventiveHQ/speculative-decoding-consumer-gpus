"""Bundle your results + auto-detected hardware into a community submission.
    ...
    python scripts/make_submission.py --name "ryzen7950x-ddr5"

Writes results/community/<name>.json: your results.json under runs.moe, plus a
...
def main():
    ...
    ap.add_argument("--name", required=True, help="short label, e.g. ryzen7950x-ddr5")
    ap.add_argument("--notes", default="", help="optional notes (RAM speed, llama.cpp version, etc.)")
    args = ap.parse_args()
    ...
    sub = {"name": args.name, "notes": args.notes,
           ...
           "runs": {"moe": json.load(open(res, encoding="utf-8-sig"))}}
